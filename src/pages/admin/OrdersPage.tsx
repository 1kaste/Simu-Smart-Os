...
const OrderShareModal: React.FC<{ order: Order, onClose: () => void }> = ({ order, onClose }) => {
    ...
    useEffect(() => {
        ...
        switch(order.status) {
            case 'Pending':
                message = `Hi ${order.customerName}, your SimuSmart order #${order.id} has been received and is being processed. ${paymentInfo}`;
                break;
            case 'Shipped':
                message = `Hi ${order.customerName}, your SimuSmart order #${order.id} has been shipped! ${paymentInfo} You can track it here: https://simusmart.example.com/track/${order.id}`;
                break;
            case 'Delivered':
                message = `Hi ${order.customerName}, your SimuSmart order #${order.id} has been delivered. We hope you enjoy your purchase! Thank you for shopping with SimuSmart.`;
                break;
            case 'Cancelled':
                message = `Hi ${order.customerName}, regarding your SimuSmart order #${order.id}, it has now been cancelled as per your request or store policy. Please contact us if you have any questions.`;
                break;
        }
    ...
    <>
      ...
      {isShareModalOpen && selectedOrder && (
          <Modal isOpen={isShareModalOpen} onClose={closeModal}>
              <OrderShareModal order={selectedOrder} onClose={closeModal} />
          </Modal>
      )}
    </>
  );
